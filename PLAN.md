...
**Status:** Draft v0.7 · Architect: Claude Opus · Editor: Nemotron-3-Super-120B-A12B (MLX, MXFP4) via LM Studio · Tool: aider (architect mode)
...
*v0.7 changes: removed stale references to the LPXVision pybind11 issue from §2 and §7. Gate 0 was cleared 2026-04-24 (commit `5fe1b5a`, tag `m1.1`); the prior "silent registration failure" was misdiagnosed and the binding has always worked when `lpximage` successfully `dlopen`'s. The §7 "pybind11 rabbit hole" risk is deleted rather than marked resolved — the risk register is a forward-looking doc, and git history + tags record the post-mortem.*
...
- **LPXVision** (C++, with Python binding as of M1.1): per-cell retinaCell pipeline that produces a 24-bit packed identifier per cell with 8 × 3-bit components — `mwh` (luminance), `hue`, and six x/y/z-direction gradients (three luminance, three hue). Accessible from Python as `lpximage.LPXVision(scanned_lpximage)`; Gate 0 cleared 2026-04-24.
...
- **M1.1 — Python access to retinaCells.** ✅ *Gate 0 cleared 2026-04-24 (commit `5fe1b5a`, tag `m1.1`).* The pybind11 binding exposes `LPXVision` from Python; instantiation and all documented properties/methods work. **Still outstanding and folded into M1.2:** the ergonomic wrapper that takes a numpy image and returns a `(151,)` uint32 array of retinaCells for a canonical view plus a `(151, 8)` unpacked uint8 tensor. That wrapper belongs in `src/lpx_adapter/data/` and is the first substantive piece of code the adapter package needs.
